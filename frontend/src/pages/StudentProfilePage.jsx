  ...
  return (
    <div className="min-h-screen bg-background text-on-background pb-28 md:pb-8">
      ...
      <header className="md:hidden sticky top-16 z-40 flex items-center justify-between px-4 h-16 bg-surface border-b border-outline-variant">
        ...
        <h1 className="text-base font-bold text-on-surface">{t('profile.mobileTitle')}</h1>
        <button
      ...
      <main className="md:hidden max-w-lg mx-auto px-4 pt-6 space-y-5">
        ...
        <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant shadow-sm p-6 flex items-center gap-5">
          ...
          {user?.studentId && (
            <div className="text-right shrink-0">
              <p className="text-[10px] text-on-surface-variant uppercase tracking-wider">{t('profile.studentId')}</p>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <div className="min-w-0">
              <p className="text-[10px] text-on-surface-variant font-bold uppercase tracking-wider">{t('profile.walletBalance')}</p>
          ...
          <div className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-4 flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-[10px] text-on-surface-variant font-bold uppercase tracking-wider">{t('profile.memberSince')}</p>
        ...
        {activeOrder && (
          <div className="bg-primary/5 border border-primary/30 rounded-2xl p-4 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-bold uppercase tracking-wider text-primary">{t('order.activeOrder')}</p>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] text-on-surface-variant font-bold uppercase tracking-wider">{t('nav.orders')}</p>
            <p className="text-sm font-bold text-on-surface">{t('order.viewOrderHistory')}</p>
          </div>
        ...
        <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant shadow-sm p-5">
          ...
          <form onSubmit={handleNameSave} className="mt-3 space-y-4">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-on-surface-variant mb-1.5">{t('profile.fullName')}</label>
              <input
                type="text"
                value={name}
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-on-surface-variant mb-1.5">{t('profile.email')}</label>
              <input
                type="email"
                disabled
                value={user?.email || ''}
              ...
              />
              <p className="text-xs text-on-surface-variant mt-1">{t('profile.emailNote')}</p>
            </div>
            <button
              type="submit"
        ...
        {!isGoogleUser && (
          <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant shadow-sm p-5">
            ...
            <form onSubmit={handlePasswordSave} className="mt-3 space-y-4">
              {[
                { key: 'current', labelKey: 'profile.currentPassword', field: 'currentPassword', placeholder: 'Enter current password' },
                { key: 'new',     labelKey: 'profile.newPassword',     field: 'newPassword',     placeholder: 'Min. 6 characters' },
                { key: 'confirm', labelKey: 'profile.confirmPassword', field: 'confirmPassword', placeholder: 'Repeat new password' },
              ].map(({ key, labelKey, field, placeholder }) => (
                <div key={key}>
                  <label className="block text-xs font-bold uppercase tracking-wider text-on-surface-variant mb-1.5">{t(labelKey)}</label>
      ...
      <div className="hidden md:grid md:grid-cols-[320px_1fr] md:gap-6 max-w-5xl mx-auto px-8 pt-4">
        ...
        <div className="space-y-5">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] text-on-surface-variant font-bold uppercase tracking-wider">{t('nav.orders')}</p>
              <p className="text-base font-bold text-on-surface">{t('order.viewOrderHistory')}</p>
              <p className="text-xs text-on-surface-variant mt-0.5">{t('order.viewOrderHistoryHint')}</p>
            </div>
          ...
          {!isGoogleUser && (
            <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant shadow-sm p-6">
              ...
              <form onSubmit={handlePasswordSave} className="mt-4 space-y-4">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-on-surface-variant mb-1.5">{t('profile.currentPassword')}</label>
                  <div className="relative">
                    <input
