...
export default function StudentOrderPage() {
  ...
  const addItemToCart = (itemId, servingSize = 'full') => {
    ...
    // Only look for the item in the active, current menu
    const sourceItem = menuItems.find((item) => String(item._id) === normalizedId)

    // If it's not on the current menu, do not let them add it to the cart
    if (!sourceItem) {
    ...
    setCart((previous) => {
      ...
      // Increment quantity if it already exists in the cart
      if (existing) {
        return previous.map((entry) => (entry === existing ? { ...entry, qty: entry.qty + 1 } : entry))
      }

      // Add new item entry to cart using the fresh data from menuItems
      return [
        ...previous,
        {
          item: {
  ...
  // Filter menu items matching search input AND selected category tab
